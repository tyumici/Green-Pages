...
Draws a random tarot card or sword. Chances are as follows:
| Outcome  | Probability % |
| -------- | ------------- |
| Upright  | 81.32%        |
| Reversed | 5.79%         |
...
Quality of the sword is then calculated as follows:
...
Presense of the handle decorations is determined after quality is rolled:
| Quality      | Conditional % |
| --------     | ------------- |
| `Common`     | 0.00%         |
| *`Artifact`* | 100.00%       |
...
All materials (handle decorum and main material of the sword), as well as the type of the sword are rolled uniformely, with the following exceptions:
- Sword of type `needle` is never rolled and is instead obtained by [!needle](#needle) command and is considered to always have handle decor of the same material.
- Sword of type `tooth` is never rolled and is instead obtained by unknown means of beating enough bandits and claiming the spoils, while its material and quality is determined by presense and skill level of a dentist in the closest town.
...
Moreover, *`artifact`* swords are guaranteed to be unique in composition - their material, type, and handle decorations, as well as the name bestowed in both [Elven](https://dwarffortresswiki.org/index.php/Elven_language) and [English](https://en.wikipedia.org/wiki/English_language) languages.

#### !armory

***Optional Parameters:*** sword_id

Displays a random sword from your armory or one specified by the `sword_id` parameter, which can be frefixed with '#'
...
Pritns the link to the game Void Stranger on steam
